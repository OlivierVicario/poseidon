...
public class BidListControllerTest {
	...
	public void givenAuthRequestOnHome_shouldSucceedWith200() throws Exception{
		this.mockMvc.perform(get("/bidList/list")).andExpect(status().isOk()).andExpect(status().is2xxSuccessful());
	}
	
	@WithMockUser(value = "spring")
	@Test
	public void givenAuthRequestOnaddBidForm_shouldSucceedWith200() throws Exception{
		this.mockMvc.perform(get("/bidList/add")).andExpect(status().isOk()).andExpect(status().is2xxSuccessful());
	}
	
	@WithMockUser(value = "spring")
	@Test
	public void givenAuthRequestOnShowUpdateForm_shouldClientErrorWith404() throws Exception{
		this.mockMvc.perform(get("/bidList/update?id=2")).andExpect(status().is4xxClientError());
	}
	
	@WithMockUser(value = "spring")
	@Test
	public void givenAuthRequestOnDeleteBid_shouldClientErrorWith404() throws Exception{
		this.mockMvc.perform(get("/bidList/delete?id=2")).andExpect(status().is4xxClientError());
	}
}
